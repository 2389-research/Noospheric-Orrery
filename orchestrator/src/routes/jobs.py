...
def list_jobs(status: str | None = None):
    ...
    if status:
        rows = conn.execute("SELECT id, type, target, status, created_at, started_at, completed_at, result FROM jobs WHERE status = ? ORDER BY created_at DESC", (status,)).fetchall()
    else:
        rows = conn.execute("SELECT id, type, target, status, created_at, started_at, completed_at, result FROM jobs ORDER BY created_at DESC").fetchall()
    conn.close()
    return [
        {
            "id": r[0], "type": r[1], "target": r[2], "status": r[3],
            "created_at": r[4], "started_at": r[5], "completed_at": r[6],
            "results": json.loads(r[7]) if r[7] and r[7].startswith("{") else None,
        }
        for r in rows
    ]
