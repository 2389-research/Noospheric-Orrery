...
async def extract_entities_from_chunk(client: AsyncAnthropic, chunk_text: str, spec: str, model: str) -> list[dict]:
...
async def extract_document(client: AsyncAnthropic, chunks: list[dict], spec: str, model: str) -> list[dict]:
    all_entities = []
    seen = set()
    for chunk in chunks:
        entities = await extract_entities_from_chunk(client=client, chunk_text=chunk["text"], spec=spec, model=model)
