...
export function StatsBar({ stats }: { stats: Stats | null }) {
  if (!stats) return <div className="text-muted-foreground/85 text-xs">Loading...</div>;
  const items = [
    ...
    { label: "ACTIVE", value: stats.active_jobs, color: stats.active_jobs > 0 ? "text-cyan-400" : "text-muted-foreground/85" },
  ];
  return (
    <div className="grid grid-cols-4 gap-4">
      {items.map((item) => (
        <div key={item.label} className="border border-border/30 rounded px-4 py-3">
          ...
          <p className="text-[9px] tracking-[3px] text-muted-foreground/90 mt-1">{item.label}</p>
        </div>
      ))}
    </div>
  );
