...
export function PipelineStages({ stats, jobs }: PipelineStagesProps) {
  ...
  const statusColor: Record<StageStatus, string> = {
    idle: "border-muted-foreground/90 text-muted-foreground/90",
        ...
        <div key={stage.name} className="flex items-center">
          ...
          {i < stages.length - 1 && (
            <div className="flex items-center mx-1">
              ...
              <div className={`text-[8px] ${stages[i + 1].status === "idle" ? "text-muted-foreground/90" : "text-muted-foreground/85"}`}>
                ▸
              </div>
            </div>
          )}
