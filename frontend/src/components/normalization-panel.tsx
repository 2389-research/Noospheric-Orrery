...
export function NormalizationPanel() {
  ...
  return (
    <div className="space-y-4">
            ...
            <div className="flex gap-3 text-[10px]">
              {Object.entries(summary.merges_by_method).map(([method, count]) => (
                <span key={method} className="flex items-center gap-1">
                  ...
                  <span className="text-muted-foreground/85">{count}</span>
                </span>
              ))}
            </div>
          )}
      ...
      {lastResult && (
        <div className="border border-border/30 rounded px-3 py-2 text-[10px] text-muted-foreground/85">{lastResult}</div>
      )}

      {visibleMerges.length > 0 && (
        <div className="space-y-2">
          <div className="rounded border border-border/30">
            <div className="grid grid-cols-[1fr_1fr_80px_60px] gap-2 px-3 py-1.5 border-b border-border/20 text-[9px] tracking-[2px] text-muted-foreground/85 uppercase">
            ...
            {visibleMerges.map((m, i) => (
              <div key={i} className="grid grid-cols-[1fr_1fr_80px_60px] gap-2 px-3 py-1.5 border-b border-border/10 last:border-0 text-xs">
                <span className="line-through text-muted-foreground/90">{m.from}</span>
                <span className="text-foreground/90">{m.to}</span>
                <Badge variant="outline" className={`w-fit text-[9px] ${methodStyle[m.method] || ""}`}>{m.method}</Badge>
                <span className="text-muted-foreground/85">{m.similarity}</span>
              </div>
            ))}
          </div>
          {mergeTotalPages > 1 && (
            <div className="flex items-center justify-end gap-1 text-xs text-muted-foreground/85">
      ...
      {visibleReviews.length > 0 && (
        <div className="space-y-2">
          <h3 className="text-[10px] tracking-[2px] text-muted-foreground/85 uppercase">
            ...
            {visibleReviews.map((r) => (
              <div key={r.id} className="flex items-center gap-3 border border-border/20 rounded px-3 py-2">
                <span className="text-xs text-foreground/85 flex-1">
                  &quot;{r.entity_a}&quot; <span className="text-muted-foreground/85">↔</span> &quot;{r.entity_b}&quot;
                </span>
                <span className="text-[10px] text-muted-foreground/90">{r.similarity}</span>
                ...
                <Button size="sm" variant="ghost" className="h-5 text-[10px] px-2 text-muted-foreground/85" onClick={() => handleResolve(r.id, "keep_separate")}>
                  skip
                </Button>
              </div>
            ))}
      ...
      {(!summary || summary.total_merges === 0) && reviews.length === 0 && !lastResult && (
        <p className="text-[10px] text-muted-foreground/90">No normalization run yet.</p>
      )}
    </div>
  );
}
