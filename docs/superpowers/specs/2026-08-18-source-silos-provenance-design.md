...
**FIVE auto-merge paths must become silo-aware — and within each, *every* sub-check that can fuse two names, not just the embedding query.** These were mis-inventoried in earlier drafts; grounded here (grep `merge_map` / `canonical_name = ? AND type = ?` / `get_by_name` before implementing, in case a sixth appears):
...
5. **`worker/src/jobs/extract_batch_image.py` (image extraction inline fuse, ~line 117)** — the image analog of path 1: the same global `SELECT id FROM entities WHERE canonical_name = ? AND type = ?`. Currently *latent* (image docs arrive via loose `/ingest` → null silo, and the text vault featurizer skips images), but it leaks the instant an image doc lands in a silo. Scope it identically, or explicitly scope image ingest out of silos with a one-line note — do not leave it unaddressed.
