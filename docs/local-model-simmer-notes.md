...
- **Known gap (TODO)**: the REDUCE is naive exact-`(name,type)` dedup, so per-chunk
  fragments/variants survive: `harper` + `harper reed`, `commer` vs `cummer` (typo),
  `vancouv`/`vancouver`, LVMH split 4 ways, an email typed as Person. Needs a
  canonicalization REDUCE — reuse the existing entity normalizer (embedding cluster +
  merge) so the golden set is high-coverage AND clean.

### Phase 2 — extraction spec (NOT yet fixed — main remaining work)
...
## Plan for the Phase 2 fix (next session)

Same principle as Phase 1: **scaffold the abstraction; don't ask the weak model to do it
all in one self-directed agentic shot.**
1. Seed the Phase 2 generator from a **rules TEMPLATE** (type-definition + empty
   INCLUDE/EXCLUDE sections + example slots), NOT the raw golden list — so it edits rules,
   never tempted to paste entities.
2. **Inline the evaluator's misses + false-positives** and ask, per type, "write an
   INCLUDE/EXCLUDE rule that fixes these" — small guided edits instead of whole-cloth
   generalization. (Also drop the agentic read-loop — inline, single-shot.)
3. **Forbid copying golden entity names** into the spec body (enforce generalizability).
4. Add the canonicalization REDUCE to Phase 1 (see gap above).
...
- `worker/src/jobs/simmer_general.py` — map-reduce golden generation (Phase 1)
- `docker-compose.ollama.yml` — local Ollama/gemma4 override (new)
- prototype: `scratchpad/mapreduce_golden.py` (standalone A/B harness, not committed)
