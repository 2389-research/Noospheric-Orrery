...
## TradeRoutePanel — detailed spec

### What a trade route represents

A trade route exists because entities appear in documents from both connected domains. The weight is the count of shared entity mentions across both domains. This is the most discovery-oriented surface in the panel — it surfaces relationships between domains that aren't visible anywhere else.

### Data needed

```javascript
// All available from GET /graph + client-side intersection:

// From graph data already in viz:
trade_route.source       // domain path
trade_route.target       // domain path
trade_route.weight       // total shared mention count

// Client-side: intersect entity lists for source and target domains
// GET /entities?domain={source} + GET /entities?domain={target}
// shared = entities appearing in both lists
// Sort shared entities by source_count descending
```

No new endpoints required. The shared entity list is a client-side set intersection on the two domain entity lists.

### Overlap percentage

The proportional stat is more legible than raw weight:

```javascript
// Show from the perspective of the smaller domain (more meaningful)
const smallerDomain = sourceEntityCount < targetEntityCount ? source : target;
const smallerCount = Math.min(sourceEntityCount, targetEntityCount);
const overlapPct = Math.round((sharedEntities.length / smallerCount) * 100);
// "15 of 84 entities in investor_meetings (18%)"
```

### Connection strength label

Translate weight into a readable signal:

```javascript
function connectionStrength(weight) {
...
Group shared entities by type, count per group, show top 2-3 entity names as examples per group. This is the most interesting single piece of information — the *kind* of things connecting two domains tells you the *nature* of the relationship.

```javascript
const byType = sharedEntities.reduce((acc, e) => {
...
┌─────────────────────────────────────────┐
│ Trade Route                             │  ← 9px label
│ strong connection · 15 entities shared  │  ← cyan accent, connection strength
│                                         │
│ ┌──────────────────┐ ┌────────────────┐ │
│ │ investor_meetings│↔│   strategy     │ │  ← each end clickable → DomainPanel
│ │    19 docs       │ │    20 docs     │ │
│ └──────────────────┘ └────────────────┘ │
│                                         │
│ 15 of 84 entities in investor_meetings  │  ← overlap stat, 10px muted
│ appear in strategy (18%)                │
│                                         │
│ WHAT CONNECTS THEM                      │  ← section label
│ ● Person    8  Harper Reed, Dylan R...  │  ← type dot + count + top 2 names
│ ● Event     4  Series B, Board Mtg...  │
│ ● Concept   3  Product-led growth...   │
│                                         │
│ SHARED ENTITIES                         │  ← section label
│ ● harper reed          12 docs    →    │  ← type dot + name + count + click
│ ● dylan richard         9 docs    →    │
│ ● series b              7 docs    →    │
│ ● betaworks             5 docs    →    │
│ ● product-market fit    3 docs    →    │
│   + 10 more                            │  ← if > 5, show count of remainder
├─────────────────────────────────────────┤
│ [⊞ filter to both domains]             │  ← footer action
└─────────────────────────────────────────┘
```

### Shared entities list

- Show top 5 by `source_count` descending
- If more than 5 shared entities exist, show "+ N more" at the bottom (not clickable — just count)
- Each row: type color dot + entity name + doc count + right arrow
- Clicking any row → EntityPanel for that entity (pushes to nav trail)
- Type dot color matches entity type color system

### "Filter to both domains" action

Navigates to `/entities?domain[]={source}&domain[]={target}&intersect=true`

This requires the entity list page to support a multi-domain intersect filter. Confirm with orrery agent whether this filter exists or needs adding. If not yet supported, the button can link to `/entities?domain={source}` as a fallback until the intersect filter lands.

### Empty state

If shared entities list is empty (weight exists but no resolvable entities):

```
No shared entities found.
The weight reflects document co-occurrence
but entity resolution is pending.
```

This can happen if the trade route weight was computed before extraction completed.

### Nav trail behavior

When a user clicks from a trade route panel into an entity or domain, push the trade route to the trail as:

```javascript
trail.push({
  name: `${sourceShort} ↔ ${targetShort}`,  // e.g. "investor_mtgs ↔ strategy"
  type: 'trade_route',
  id: `${source}|${target}`
});
```

Clicking back in the trail re-renders the trade route panel for that pair.
